...
// Get income grouped by category for the month
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $total_month_income += floatval($row['total_amount']);
}
...
foreach ($categories as &$cat) {
    $cat['percentage'] = $total_month_income > 0 ? ($cat['amount'] / $total_month_income) * 100 : 0;
}

// Get daily income for the bar chart
...
if ($stmt_daily) {
    ...
    while ($row = mysqli_fetch_assoc($result_daily)) {
        $daily_income[intval($row['day'])] = floatval($row['daily_total']);
...
echo json_encode([
    ...
    'total_income' => floatval($total_month_income),
